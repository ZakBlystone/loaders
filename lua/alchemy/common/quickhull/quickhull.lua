...
local vertexList = include("vertexlist.lua")
local vertex = include("vertex.lua")
local face = include("face.lua")
...
local quickHull = {};
local quickHull_mt = {__index = quickHull};
...
function quickHull.new(points)
	...
	return setmetatable(self, quickHull_mt);
end;

function quickHull.quickRun(points)
	local triangles = {};
	
	local qh = quickHull.new(points);
	qh:build();
